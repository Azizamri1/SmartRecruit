...
_MUST_PAT = re.compile(r"\b(must|mandatory|required|obligatoire|nécessaire|necessaire|requis)\b", re.I)
...
def parse_profile_requirements(text: Optional[str], *, job_skills: Optional[List[str]] = None) -> dict:
    ...
    for p in parts:
        if _MUST_PAT.search(p):
            ...
            techs = sorted((toks & job_skill_tokens)) if job_skill_tokens else list(toks)
