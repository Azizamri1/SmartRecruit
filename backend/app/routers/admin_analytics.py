...
def _summary(db: Session):
    ...
    open_jobs = (
        db.query(models.Job)
        .filter(or_(models.Job.deadline == None, models.Job.deadline >= date.today()))
        .count()
    )
    ...
    status_result = db.execute(
        text("SELECT status, COUNT(*) as count FROM applications GROUP BY status")
    ).fetchall()
    by_status = {str(s) if s else "unknown": int(c) for s, c in status_result}

    # Get scores
    scores_result = db.execute(
        text("SELECT score FROM applications WHERE score IS NOT NULL")
    ).fetchall()
